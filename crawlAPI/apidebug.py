    ...
    if int(apitype)==1:
        try:
            ...
            if categorysinfos!={}:
                ...
                return 301,'审核失败:分类接口内容获取为空,接口返回内容格式不标准!'
        except Exception, e:
            return 401,'审核失败:接口无效或者接口类型选择有误!'
        ...
        try:
            ...
            if len(bookinfos.keys())>1:###如果接口获取失败则返回值是{'mcpid':mcpid}，故用次方法判读接口获取是否成功
                ...
                return 303,'审核失败:图书信息接口内容获取为空，接口返回内容格式不标准!'
        except Exception, e:
            return 401,'审核失败:接口无效或者接口类型选择有误!'
        ...
        try:    
            ...
            if cidsinfos!=[]:
                ...
                return 304,'审核失败:章节信息接口内容获取为空，接口返回内容格式不标准!'
        except Exception, e:
            return 401,'审核失败:接口无效或者接口类型选择有误!'
        ...
        try:
            ...
            if wordsize!=0:
                ...
                return 305,'审核失败:章节内容接口内容获取为空，接口返回内容格式不标准!'
        except Exception, e:
            return 401,'审核失败:接口无效或者接口类型选择有误!'
